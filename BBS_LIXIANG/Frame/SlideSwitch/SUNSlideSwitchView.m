...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        NSDictionary *attribute = @{NSFontAttributeName: [UIFont systemFontOfSize:kFontSizeOfTabButton]};
        CGSize size = [vc.title  boundingRectWithSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView) options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;

        //累计每个tab文字的长度
        topScrollViewContentWidth += kWidthOfButtonMargin+size.width;
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset,0,
                                    size.width, kHeightOfTopScrollView)];
        //计算下一个tab的x偏移量
        xOffset += size.width + kWidthOfButtonMargin;
        
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0, size.width, _shadowImage.size.height);
            button.selected = YES;
        }
